...
class IDD(torch.utils.data.Dataset):
    def __init__(self,list_img_path,list_anno_path,transforms=None):
        ...
        self.classes = {'person':0,'rider':1,'car':2,'truck':3,
    ...
    def get_label_bboxes(self,xml_obj):
        ...
        for node in xml_obj.getroot().iter('object'):
            ...
            if object_present in self.classes:
...
class IDD_Test(torch.utils.data.Dataset):
    def __init__(self,list_img_path,list_anno_path):
        super(IDD_Test,self).__init__()
        self.img = sorted(list_img_path)
        self.anno = sorted(list_anno_path)
        self.classes = {'person':0,'rider':1,'car':2,'truck':3,
         'bus':4,'motorcycle':5,'bicycle':6,'autorickshaw':7,'animal':8,'traffic light':9,
          'traffic sign':10,'vehicle fallback':11,'caravan':12,'trailer':13,'train':14}
    ...
    def get_height_and_width(self,idx):
        img_path = os.path.join(img_path,self.imgs[idx])
        img = Image.open(img_path).convert("RGB")
        dim_tensor = torchvision.transforms.ToTensor()(img).shape
        height,width = dim_tensor[1],dim_tensor[2]
        return height,width
